import React from 'react';
import { useDispatch, useSelector } from 'react-redux';
import { setSorting } from '../../redux/slices/filterSlice';

export default function Sorting() {
  const [isOpen, setIsOpen] = React.useState(false);
  const sortRef = React.useRef();
  const sorting = useSelector((state) => state.filter.sorting);
  const dispatch = useDispatch();
  const list = [
    { name: 'популярности по убыванию', sortProperty: 'rarity', order: 'desc' },
    { name: 'цене по убыванию', sortProperty: 'price', order: 'desc' },
    { name: 'алфавиту по убыванию', sortProperty: 'title', order: 'desc' },
    { name: 'популярности по возрастанию', sortProperty: 'rarity', order: 'asc' },
    { name: 'цене по возрастанию', sortProperty: 'price', order: 'asc' },
    { name: 'алфавиту по возрастанию', sortProperty: 'title', order: 'asc' },
  ];

  const changeSort = (sort) => {
    setIsOpen(false);
    dispatch(setSorting(sort));
  };
  React.useEffect(() => {
    const handleClickOutside = (e) => {
      if (!e.composedPath().includes(sortRef.current)) {
        setIsOpen(false);
      }
    };
    document.body.addEventListener('click', handleClickOutside);
    return () => {
      document.body.removeEventListener('click', handleClickOutside);
    };
  }, []);
  return (
    <>
      <div ref={sortRef} class='sort'>
        <div class='sort__label'>
          <svg
            width='10'
            height='6'
            viewBox='0 0 10 6'
            fill='none'
            xmlns='http://www.w3.org/2000/svg'
          >
            <path
              d='M10 5C10 5.16927 9.93815 5.31576 9.81445 5.43945C9.69075 5.56315 9.54427 5.625 9.375 5.625H0.625C0.455729 5.625 0.309245 5.56315 0.185547 5.43945C0.061849 5.31576 0 5.16927 0 5C0 4.83073 0.061849 4.68424 0.185547 4.56055L4.56055 0.185547C4.68424 0.061849 4.83073 0 5 0C5.16927 0 5.31576 0.061849 5.43945 0.185547L9.81445 4.56055C9.93815 4.68424 10 4.83073 10 5Z'
              fill='#2C2C2C'
            ></path>
          </svg>
          <b>Сортировка по:</b>
          <span onClick={() => setIsOpen(!isOpen)}>{sorting.name}</span>
        </div>
        {isOpen ? (
          <div class='sort__popup'>
            <ul>
              {list.map((obj, index) => (
                <li
                  key={index}
                  onClick={() => changeSort(obj)}
                  class={sorting.name === obj.name ? 'active' : ''}
                >
                  {obj.name}
                </li>
              ))}
            </ul>
          </div>
        ) : (
          ''
        )}
      </div>
    </>
  );
}
