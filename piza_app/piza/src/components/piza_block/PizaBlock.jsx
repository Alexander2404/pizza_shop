import React from 'react';
import { useDispatch, useSelector } from 'react-redux';
import { addItem } from '../../redux/slices/cartSlice';

export default function PizaBlock({ id, title, price, imageUrl, sizes, types }) {
  const items = useSelector((state) => state.cart.items.filter((obj) => obj.id === id));

  const [typeActive, setTypeActive] = React.useState(types[0]);
  const [sizeActive, setSizeActive] = React.useState(0);
  const typeNames = ['тонкое', 'традиционное'];

  const pizaCount = items.reduce((sum, obj) => {
    return obj.count + sum;
  }, 0);
  const dispatch = useDispatch();
  React.useEffect(() => {}, [items]);

  const onClickAddItem = () => {
    dispatch(
      addItem({
        id,
        title,
        price,
        imageUrl,
        size: sizes[sizeActive],
        type: types[typeActive],
      }),
    );
  };

  return (
    <div className='pizza-block-wrapper'>
      <div class='pizza-block'>
        <img class='pizza-block__image' src={imageUrl} alt='Pizza' />
        <h4 class='pizza-block__title'>{title}</h4>
        <div class='pizza-block__selector'>
          <ul>
            {types.map((type, index) => (
              <li
                key={index}
                onClick={() => setTypeActive(type)}
                className={type === typeActive ? 'active' : ''}
              >
                {typeNames[type]}
              </li>
            ))}
          </ul>
          <ul>
            {sizes.map((size, index) => (
              <li
                key={index}
                onClick={() => setSizeActive(index)}
                className={sizeActive === index ? 'active' : ''}
              >
                {size} см.
              </li>
            ))}
          </ul>
        </div>
        <div class='pizza-block__bottom' onClick={onClickAddItem}>
          <div class='pizza-block__price'>от {price} ₽</div>
          <div class='button button--outline button--add'>
            <svg
              width='12'
              height='12'
              viewBox='0 0 12 12'
              fill='none'
              xmlns='http://www.w3.org/2000/svg'
            >
              <path
                d='M10.8 4.8H7.2V1.2C7.2 0.5373 6.6627 0 6 0C5.3373 0 4.8 0.5373 4.8 1.2V4.8H1.2C0.5373 4.8 0 5.3373 0 6C0 6.6627 0.5373 7.2 1.2 7.2H4.8V10.8C4.8 11.4627 5.3373 12 6 12C6.6627 12 7.2 11.4627 7.2 10.8V7.2H10.8C11.4627 7.2 12 6.6627 12 6C12 5.3373 11.4627 4.8 10.8 4.8Z'
                fill='white'
              ></path>
            </svg>
            <span>Добавить</span>

            <i>{pizaCount}</i>
          </div>
        </div>
      </div>
    </div>
  );
}
